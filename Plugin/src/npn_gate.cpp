...
void NPN_PluginThreadAsyncCall(NPP npp, void (*func)(void *), void *userData)
{
  ...
  OneShotManager::getInstance().push(userData, func);

#elif defined(XP_WIN) && !defined(CHROME)

  CPlugin * pPlugin = (CPlugin *)(npp->pdata);
  if (pPlugin->m_win_message && pPlugin->m_isSafari)
    pPlugin->m_win_message->_scheduleAsyncCall(npp, func, userData);
  else
